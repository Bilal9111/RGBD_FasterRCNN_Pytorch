...
from model.faster_rcnn.faster_rcnn_rgbd_early import _fasterRCNN
...
def resnet50(pretrained=False):
  ...
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = ResNet(Bottleneck, [3, 4, 12, 3])
  if pretrained:
    model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
  return model

def resnet50_Depth(pretrained=False):
  """Constructs a ResNet-50 model.
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = ResNet_Depth(Bottleneck, [3, 4, 6, 3])
  if pretrained:
    model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
  return model
...
class resnet(_fasterRCNN): # The _fasterRCNN is basically means that resnet class is a derived class which uses _fasterRCNN as a base class
  ...
  def _init_modules(self):
    ...
    if self.pretrained == True:
      ...
      depth_conv1_weight = state_dict["conv1.weight"]
      depth_conv1_weight = depth_conv1_weight[:,0:1,:]
      state_dict["conv1.weight"] = depth_conv1_weight
      
      resnet_d.load_state_dict({k:v for k,v in state_dict.items() if k in resnet_d.state_dict()},strict=False)
    ...
    self.RCNN_base = nn.Sequential(resnet.conv1, resnet.bn1,resnet.relu,
      resnet.maxpool,resnet.layer1[0]) # Defining the RCNN_base which is later used by the functions in the base class _fasterRCNN
    
    
    self.RCNN_base2 = nn.Sequential(resnet_d.conv1, resnet_d.bn1,resnet_d.relu,
      resnet_d.maxpool,resnet_d.layer1) # Defining the RCNN_base which is later used by the functions in the base class _fasterRCNN
    
    
    
    self.RCNN_postprocess = nn.Sequential(resnet.layer1[1],resnet.layer1[2],resnet.layer2,resnet.layer3)
