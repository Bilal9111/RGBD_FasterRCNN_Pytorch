...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data_a, im_info_a, gt_boxes_a, num_boxes_a, im_data_b, im_info_b, gt_boxes_b, num_boxes_b):
        ...
        #print(torch.equal(im_data_a,im_data_b))
        ...
        im_data_b = im_data_b[:,0:1,:] # Trim the Depth data tensor
        
       


        # feed image data to base model to obtain base feature map
        rgb_feat = self.RCNN_preprocess_a(im_data_a) # Conv5
        depth_feat = self.RCNN_preprocess_b(im_data_b) # Conv5
        conc_feat = rgb_feat#+ (0.25*depth_feat) #torch.cat((rgb_feat,depth_feat),1) 
        base_feat = self.RCNN_postprocess(conc_feat)
