...
class ResNet_Merge(nn.Module):
  def __init__(self, block, layers, num_classes=1000):
    ...
    self.inplanes = 64
    self.b_conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,bias=False)
    self.b_bn1 = nn.BatchNorm2d(64)
    self.b_relu = nn.ReLU(inplace=True)
    self.b_maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=0, ceil_mode=True)
    self.b_layer1 = self._make_layer(block, 64, 1) 
    ...
    # Inplanes adjust after concatenation
...
class resnet(_fasterRCNN): 
  ...
  def _init_modules(self):
    ...
    if self.pretrained == True:
      ...
      depth_conv1_weight = state_dict["b_conv1.weight"]
      depth_conv1_weight = depth_conv1_weight[:,0:1,:]
      state_dict["b_conv1.weight"] = depth_conv1_weight
    ...
    self.RCNN_preprocess_a = nn.Sequential(resnet.a_conv1, resnet.a_bn1,resnet.a_relu,
      resnet.a_maxpool,resnet.a_layer1[0]) # RGB path Conv5
      
    self.RCNN_preprocess_b = nn.Sequential(resnet.b_conv1, resnet.b_bn1,resnet.b_relu,
      resnet.b_maxpool,resnet.b_layer1) # Depth path Conv5
    
    # Postprocess
    self.RCNN_postprocess = nn.Sequential(resnet.a_layer1[1],resnet.a_layer1[2],resnet.layer2,resnet.layer3)
