...
class kittivoc_depth(imdb):
    ...
    def image_depth_path_at(self, i):
        """
        Return the absolute path to image i in the image sequence.
        """
        return self.image_depth_path_from_index(self._image_index[i])
    ...
    def image_path_from_index(self, index):
        ...
        :return filepath
        """
        image_path = os.path.join(self._data_path, 'standard_stereo',
    ...
    def gt_roidb(self):
        ...
        """
        cache_file = os.path.join(self.cache_path, self.name + '_depth_gt_roidb.pkl')
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        for i, cls in enumerate(self._classes):
            ...
            track_file.write('AP for {} = {:.4f}\n'.format(cls, ap))
            with open(os.path.join(output_dir, cls + '_pr.pkl'), 'w') as f:
                cPickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)
        print('Mean AP = {:.4f}'.format(np.mean(aps)))
        track_file.write('Mean AP = {:.4f}\n'.format(np.mean(aps)))
        track_file.close()
        print('~~~~~~~~')
        print('Results:')
        for ap in aps:
