...
class ResNet(nn.Module):
  def __init__(self, block, layers, num_classes=1000):
    ...
    #self.fc_b = nn.Linear(512 * block.expansion, num_classes)
...
class resnet(_fasterRCNN): # The _fasterRCNN is basically means that resnet class is a derived class which uses _fasterRCNN as a base class
  ...
  def _init_modules(self):
    ...
    """
    self.RCNN_base = nn.Sequential(resnet.conv1_a, resnet.bn_a,resnet.relu_a,
      resnet.maxpool_a,resnet.layer1_a,resnet.layer2_a,resnet.layer3_a,resnet.conv1_b, resnet.bn_b,resnet.relu_b,
      resnet.maxpool_b,resnet.layer1_b,resnet.layer2_b,resnet.layer3_b) # Defining the RCNN_base which is later used by the functions in the base class _fasterRCNN

    self.RCNN_top = nn.Sequential(resnet.layer4_a,resnet.layer4_b)  # Defining the RCNN_top which is later used by the functions in the base class _fasterRCNN
    ...
    if cfg.RESNET.FIXED_BLOCKS >= 3:
      ...
      for p in self.RCNN_base[13].parameters(): p.requires_grad=False
    if cfg.RESNET.FIXED_BLOCKS >= 2:
      for p in self.RCNN_base[5].parameters(): p.requires_grad=False
      for p in self.RCNN_base[12].parameters(): p.requires_grad=False
    if cfg.RESNET.FIXED_BLOCKS >= 1:
      for p in self.RCNN_base[4].parameters(): p.requires_grad=False
      for p in self.RCNN_base[11].parameters(): p.requires_grad=False
